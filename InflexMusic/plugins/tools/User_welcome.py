...
from InflexMusic.utils.raja_ban import admin_filter
import os
...
def welcomepic(pic, user, chatname, id, uname, brightness_factor=1.3):
    background = Image.open("InflexMusic/assets/wel2.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp, brightness_factor=brightness_factor) 
    pfp = pfp.resize((825, 824))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('InflexMusic/assets/font.ttf', size=110)
    welcome_font = ImageFont.truetype('InflexMusic/assets/font.ttf', size=60)
...
async def greet_new_members(_, member: ChatMemberUpdated):
    try:
        ...
        if member.new_chat_member and not member.old_chat_member:
            welcome_text = f"""**Wᴇʟᴄᴏᴍᴇ Dᴇᴀʀ** {user.mention}\n**@{user.username}**"""
            await asyncio.sleep(3) 
